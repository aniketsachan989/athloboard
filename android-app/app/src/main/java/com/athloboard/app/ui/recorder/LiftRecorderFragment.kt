...
class LiftRecorderFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val liftTypes = arrayOf("Squat", "Bench Press", "Deadlift")
    ...
    private fun submitRecordedLift() {
        ...
        val weight = binding.etClaimedWeight.text.toString().toIntOrNull() ?: 265

        val submission = AthloRepository.submitLiftVideo(liftType, weight)

        Toast.makeText(
            requireContext(),
            "Lift of ${submission.claimedWeight}kg ($liftType) Submitted to Super Admin Queue!",
            Toast.LENGTH_LONG
        ).show()
